...
if (isset($_REQUEST['pid'])) {
	...
	$pid = mysqli_real_escape_string($connect,$_REQUEST['pid']);
}else {
	header('location: index.php');
}


$getposts = mysqli_query($connect,"SELECT * FROM products WHERE id ='$pid'") or die(mysql_error());
...
<body>
	...
	<div style="padding: 30px 95px; font-size: 25px; margin: 0 auto; display: table; width: 98%;">
		...
		<div>
		<?php 
			$getposts = mysqli_query($connect,"SELECT * FROM products WHERE available >='1' AND id != '".$pid."' AND item ='".$item."'  ORDER BY RAND() LIMIT 3") or die(mysql_error());
					if (mysqli_num_rows($getposts)) {
					echo '<ul id="recs">';
