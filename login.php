...
if (isset($_POST['login'])) {
	if (isset($_POST['email']) && isset($_POST['password'])) {
		$user_login = mysqli_real_escape_string($connect,$_POST['email']);
		$user_login = mb_convert_case($user_login, MB_CASE_LOWER, "UTF-8");	
		$password_login = mysqli_real_escape_string($connect,$_POST['password']);		
		$num = 0;
		$password_login_md5 = md5($password_login);
		$result = mysqli_query($connect,"SELECT * FROM user WHERE (email='$user_login') AND password='$password_login_md5' AND activation='yes'");
		...
		if ($num>0) {
			...
			if (isset($_REQUEST['ono'])) {
				$ono = mysqli_real_escape_string($connect,$_REQUEST['ono']);
			...
			$result1 = mysqli_query($connect,"SELECT * FROM user WHERE (email='$user_login') AND password='$password_login_md5' AND activation='no'");
...
if(isset($_POST['activate'])){
	if(isset($_POST['actcode'])){
		$user_login = mysqli_real_escape_string($connect,$_POST['acemail']);
		$user_login = mb_convert_case($user_login, MB_CASE_LOWER, "UTF-8");	
		$user_acccode = mysqli_real_escape_string($connect,$_POST['actcode']);
		$result2 = mysqli_query($connect,"SELECT * FROM user WHERE (email='$user_login') AND confirmCode='$user_acccode'");
		...
		if ($num3>0) {
			...
			$_SESSION['user_login'] = $get_user_uname_db;
			setcookie('user_login', $user_login, time() + (365 * 24 * 60 * 60), "/");
			mysqli_query($connect,"UPDATE user SET confirmCode='0', activation='yes' WHERE email='$user_login'");
			if (isset($_REQUEST['ono'])) {
				$ono = mysqli_real_escape_string($connect,$_REQUEST['ono']);
				header("location: orderform.php?poid=".$ono."");
			}else {
...
<html>
	...
	<body class="home-welcome-text" style="background-image: url(image/homebackgrndimg1.png);">
		<div class="homepageheader" style="background-color:white">
