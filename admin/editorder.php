...
if (!isset($_SESSION['admin_login'])) {
	...
	if (isset($_REQUEST['eoid'])) {
		...
		$getposts5 = mysqli_query($connect,"SELECT * FROM orders WHERE id='$eoid'") or die(mysql_error());
	...
	$result1 = mysqli_query($connect,"SELECT * FROM orders WHERE id='$eoid'");
		$get_order_info = mysqli_fetch_assoc($result1);
			...
			$result2 = mysqli_query($connect,"SELECT * FROM user WHERE id='$eouid'");
...
$getposts = mysqli_query($connect,"SELECT * FROM products WHERE id ='$eopid'") or die(mysql_error());
	...
	try {
		if(empty($_POST['dstatus'])) {
			throw new Exception('Status can not be empty');
				...
				if(mysqli_query($connect,"UPDATE orders SET dstatus='$eodstatus', ddate='$ddate', quantity='$dquantity' WHERE id='$eoid'")){
					//success message
...
//triming name
	if(mysqli_query($connect,"DELETE FROM orders WHERE id='$eoid'")){
