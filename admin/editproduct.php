...
if (!isset($_SESSION['admin_login'])) {
	...
	$result = mysqli_query($connect,"SELECT * FROM admin WHERE id='$user'");
	$get_user_email = mysqli_fetch_assoc($result);
		$uname_db = $get_user_email['firstName'];

}
$getposts = mysqli_query($connect,"SELECT * FROM products WHERE id ='$epid'") or die(mysql_error());
...
if (isset($_POST['updatepro'])) {
	...
	if($result = mysqli_query($connect,"UPDATE products SET pName='$_POST[pname]',price='$_POST[price]',description='$_POST[descri]',available='$_POST[available]',category='$_POST[category]',type='$_POST[type]',item='$_POST[item]',pCode='$_POST[code]' WHERE id='$epid'")){
...
//triming name
	$getposts1 = mysqli_query($connect,"SELECT pid FROM orders WHERE pid='$epid'") or die(mysql_error());
					if ($ttl = mysql_num_rows($getposts1)) {
						$error_message = "You can not delete this product.<br>Someone ordered this.";
					}
					else {
						if(mysqli_query($connect,"DELETE FROM products WHERE id='$epid'")){
								...
								<div class="signup_form">
									...
									<div>
										...
										font-style: italic;margin-bottom: 3px;margin-top: 0px;padding: 14px;line-height: 25px;border-radius: 4px;border: 1px solid #169E8F;color: #169E8F;margin-left: 0;width: 300px;background-color: transparent;" class="">
												...
												<option value="hijab">Kurthi</option>
